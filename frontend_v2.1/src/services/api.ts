...
import type { ApiResponse, Chamber, Experiment } from '@/types'
...
class ApiService {
  ...
  async createExperiment(data: Experiment): Promise<ApiResponse<Experiment>>{
    const response = await this.api.post('/experiments', data)
    return response.data
  }

  async updateExperiment(id: string, data: Partial<Experiment>): Promise<ApiResponse<Experiment>> {
  ...
  async getMyChambersAccess(): Promise<ApiResponse<Chamber[]>> {
