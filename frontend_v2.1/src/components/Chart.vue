<template>
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      <div class="mb-4">
        <div class="flex items-center justify-between mb-2">
          <h5 class="text-lg font-medium text-gray-700">{{ title }}</h5>
          <div class="flex items-center gap-2">
            ...
            <button
              @click.prevent="showData"
              class="px-3 py-2 text-base text-blue-600 hover:text-blue-700 border border-blue-300 rounded hover:bg-blue-50 transition-colors"
            >
              View Data
            </button>
          </div>
  ...
  function showData() {
    if (Object.keys(selectedPoints.value).length === 0) {
      alert('No temperature points selected')
      return
    }
    
    let message = 'Temperature Schedule:\n\n'
    sortedSelectedDays.value.forEach(day => {
      message += `Day ${day}: ${selectedPoints.value[day]}°C\n`
    })
